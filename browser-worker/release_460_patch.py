...
def patch_audit(text: str) -> str:
    if 'from best_practice_integration_assessment import build_best_practice_assessment' not in text:
        ...
        if marker not in text:
            raise RuntimeError('4.6.0 active analysis import anchor not found')
        text = text.replace(
            marker,
...
'''
    if 'best_practice_assessment = build_best_practice_assessment(' not in text:
        if anchor not in text:
            raise RuntimeError('4.6.0 data checks anchor not found')
...
def patch_dashboard(text: str) -> str:
    ...
    text = text.replace(
        "['Findings',sum.findings||0]",
        "['Оценка',ass.overall_score??'—'],['Критично',(ass.severity_summary||{}).critical||0],['Findings',sum.findings||0]",
        1,
    )
    ...
    old_conclusion = "const tc=p.technical_conclusion||{},f=p.findings||[];q('#findings').innerHTML=panel('Техническое заключение'"
    if old_conclusion in text:
        start = text.index(old_conclusion)
        end_marker = "}`\nasync function load"
        end = text.find(end_marker, start)
        if end != -1:
            replacement = '''const tc=p.technical_conclusion||{},f=bpFindings;q('#findings').innerHTML=panel('Техническое заключение',`<div style="padding:14px"><div class="row"><span>Вердикт</span>${badge(ass.verdict||tc.status)}</div><div class="row"><span>Допуск к эксплуатации</span><b>${esc(ass.industrial_admission||tc.industrial_admission||'—')}</b></div><div class="row"><span>Общая оценка</span><b>${esc(ass.overall_score??tc.overall_score??'—')}</b></div><div class="row"><span>Подтверждённых отклонений</span><b>${f.length}</b></div><p>${esc(tc.reason||'')}</p></div>`)+panel('Подтверждённые отклонения',table(['Severity','Проблема','Факт','Влияние','Исправление'],f.map(x=>`<tr><td>${badge(x.severity)}</td><td><b>${esc(x.title||x.area)}</b></td><td>${esc(x.fact)}</td><td>${esc(x.impact||'')}</td><td>${esc(x.recommendation||x.action||'')}</td></tr>`)),`${f.length} findings`)+panel('План доработок',table(['#','Severity','Задача','Основание','Критерий приёмки'],recs.map(x=>`<tr><td class="mono">${esc(x.priority)}</td><td>${badge(x.severity)}</td><td><b>${esc(x.task)}</b></td><td>${esc(x.basis||'')}</td><td>${esc(x.acceptance||'')}</td></tr>`)),`${recs.length} actions`)}
'''
...
def main() -> None:
    ...
    if MANIFEST.exists():
        ...
        manifest = manifest.replace('EDITION = "Active Integration Analysis"', 'EDITION = "Best-Practice Integration Assessment"')
