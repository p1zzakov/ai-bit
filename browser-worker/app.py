...
settings = Settings()
app = FastAPI(title="AI-BIT Browser Worker", version="0.1.0")
PLAYWRIGHT = None
...
async def login(force: bool = False) -> dict[str, Any]:
    ...
    try:
        target = urljoin(settings.browser_base_url.rstrip("/") + "/", settings.browser_login_path.lstrip("/"))
        ...
        login_input = page.locator('input[name="USER_LOGIN"], input[name="login"], input[type="email"]').first
        password_input = page.locator('input[name="USER_PASSWORD"], input[name="password"], input[type="password"]').first
        if await login_input.count() == 0 or await password_input.count() == 0:
            raise HTTPException(status_code=422, detail="Login form was not recognized. Interactive/SSO login may be enabled.")

        await login_input.fill(settings.browser_login)
        await password_input.fill(settings.browser_password)
        submit = page.locator('button[type="submit"], input[type="submit"], button:has-text("Войти"), button:has-text("Log in")').first
...
async def collect_page(page: Page, name: str, full_page: bool) -> dict[str, Any]:
    stamp = utc_stamp()
    ...
    menu_text = await page.locator("nav, aside, [class*=menu], [class*=sidebar], [class*=navigation]").all_inner_texts()
    links = await page.locator("a[href]").evaluate_all("els => els.slice(0,500).map(a => ({text:(a.innerText||'').trim(), href:a.href})).filter(x => x.text || x.href)")
    ...
    evidence = {
        ...
        "artifacts": {"screenshot": str(screenshot_path), "html": str(html_path), "evidence": str(evidence_path)},
    }
...
async def scan_one(request: ScanRequest) -> dict[str, Any]:
    ...
    page = await context.new_page()
    try:
        target = urljoin(settings.browser_base_url.rstrip("/") + "/", request.path.lstrip("/"))
        if not same_origin(target):
            raise HTTPException(status_code=400, detail="Only the configured Bitrix24 origin is allowed")
        await page.goto(target, wait_until="domcontentloaded")
        if not await is_authenticated(page):
            await context.close()
            await login()
            ...
            await page.goto(target, wait_until="domcontentloaded")
        if request.wait_for:
            await page.locator(request.wait_for).first.wait_for(state="visible")
        try:
            await page.wait_for_load_state("networkidle", timeout=15000)
        except Exception:
            pass
        return await collect_page(page, request.name, request.full_page)
    finally:
        await context.close()
...
async def health() -> dict[str, Any]:
    return {"status": "ok", "service": "AI-BIT Browser Worker", "version": "0.1.0", "configured": bool(settings.browser_base_url), "session_saved": settings.browser_state_file.exists()}
...
async def scan_batch(request: BatchScanRequest) -> dict[str, Any]:
    ...
    return {"status": "completed" if not errors else "completed_with_errors", "results": results, "errors": errors}


@app.post("/scan/all")
async def scan_all() -> dict[str, Any]:
    return await scan_batch(BatchScanRequest(pages=[ScanRequest(name=name, path=path) for name, path in READ_ONLY_PRESETS.items()]))
