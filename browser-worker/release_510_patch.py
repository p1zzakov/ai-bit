...
def save(p,event=None):
 ...
 project_path(p['id']).write_text(json.dumps(p,ensure_ascii=False,indent=2),encoding='utf-8')
 (VERSIONS/f"{p['id']}-r{int(p.get('revision',1)):04d}.json").write_text(json.dumps(p,ensure_ascii=False,indent=2),encoding='utf-8')
 return p
def parse(path):
 s=path.suffix.lower()
 if s=='.docx':
  d=Document(path);pars=[p.text.strip() for p in d.paragraphs if p.text.strip()];tabs=[[[c.text.strip() for c in r.cells] for r in t.rows[:200]] for t in d.tables[:30]]
  return {'format':'docx','text':'\n'.join(pars)[:300000],'paragraphs':pars[:2000],'tables':tabs}
 if s=='.pdf':
...
def extract_requirements(materials):
 ...
 return out[:500]
def detect_conflicts(reqs,corpus):
 out=[];low=corpus.casefold()
 for pair,title in [(("текущий остаток","остаток на дату"),'Не определён момент остатка'),(("плановая прибыль","фактическая прибыль"),'Не определён вид прибыли'),(("по заказам","по реализациям"),'Разные документы-основания')]:
  if all(x in low for x in pair):out.append({'id':f'CON-{len(out)+1:03d}','severity':'high','title':title,'evidence':list(pair),'resolution_required':True})
 return out
def build_questions(p):
 rows=[('goal','Цель','Какое решение должно приниматься по результату?',1),('users','Пользователи','Кто будет пользоваться результатом?',1),('source_of_truth','Данные','Что считается источником правильных данных?',1),('period','Период','Какая дата основная и за какой период строится результат?',1),('filters','Отборы','Какие отборы и исключения нужны?',0),('calculations','Расчёты','Какие показатели и по каким формулам рассчитываются?',1),('acceptance','Приёмка','На каком примере доказать корректность?',1),('performance','Нагрузка','Какой объём и допустимое время выполнения?',0)]
 low='\n'.join(material_text(x) for x in p.get('materials',[])).casefold()
 if any(x in low for x in ('прибыл','марж','себестоим')):rows.append(('profit_logic','Расчёты','Прибыль плановая или фактическая и откуда берётся себестоимость?',1))
 if 'остат' in low:rows.append(('stock_logic','Расчёты','Остаток текущий или на дату, учитывать ли резерв?',1))
 for c in p.get('conflicts',[]):rows.append((f"resolve_{c['id']}",'Конфликт',f"Уточните: {c['title']}. Какой вариант правильный?",1))
 return [{'id':i,'section':s,'question':q,'required':bool(r)} for i,s,q,r in rows]
def latest_snapshot():
...
def assess(p):
 corpus='\n'.join(material_text(x) for x in p.get('materials',[]))+json.dumps(p.get('answers',{}),ensure_ascii=False);low=corpus.casefold();snap=latest_snapshot();st=json.dumps(snap['payload'],ensure_ascii=False).casefold();cand=[]
...
def markdown(d):return '# '+d.get('document','Документ')+'\n\n```json\n'+json.dumps(d,ensure_ascii=False,indent=2)+'\n```\n'
...
async def create_project(title:str=Form(...),project_type:str=Form('onec_report'),description:str=Form(''),files:list[UploadFile]=File(default=[])):
 i=uuid.uuid4().hex;materials=[];total=0
 for n,u in enumerate(files[:30]):
  ...
  name=re.sub(r'[^A-Za-zА-Яа-яЁё0-9._-]','_',Path(u.filename).name);target=UPLOADS/f'{i}-{n:02d}-{name}';target.write_bytes(b);materials.append({'id':uuid.uuid4().hex,'filename':u.filename,'size':len(b),'sha256':hashlib.sha256(b).hexdigest(),'analysis':parse(target)})
 reqs=extract_requirements(materials);corpus='\n'.join(material_text(x) for x in materials);p={'id':i,'version':VERSION,'revision':1,'title':title,'project_type':project_type,'project_type_label':TYPES.get(project_type,project_type),'description':description,'status':'interview_required','created_at':now(),'materials':materials,'requirements':reqs,'conflicts':detect_conflicts(reqs,corpus),'answers':{},'history':[]};p['questions']=build_questions(p);return save(p,'project_created')
@router.post('/api/projects/{i}/materials')
async def add_materials(i:str,files:list[UploadFile]=File(...)):
 p=load(i)
 for u in files[:30]:
  b=await u.read();name=re.sub(r'[^A-Za-zА-Яа-яЁё0-9._-]','_',Path(u.filename or 'file').name);target=UPLOADS/f'{i}-{uuid.uuid4().hex[:8]}-{name}';target.write_bytes(b);p['materials'].append({'id':uuid.uuid4().hex,'filename':u.filename,'size':len(b),'sha256':hashlib.sha256(b).hexdigest(),'analysis':parse(target)})
 p['requirements']=extract_requirements(p['materials']);p['conflicts']=detect_conflicts(p['requirements'],'\n'.join(material_text(x) for x in p['materials']));p['questions']=build_questions(p);p['revision']+=1;return save(p,'materials_added')
@router.get('/api/projects/{i}')
def get_project(i:str):return load(i)
@router.post('/api/projects/{i}/answers')
def submit_answers(i:str,r:Answers):
...
def approve(i:str,r:Decision):
 p=load(i);p['approval']={'approved':r.approved,'comment':r.comment,'at':now()};p['documents']=build_documents(p);p['status']='documents_ready' if r.approved else 'revision_required';return save(p,'project_decision')
@router.get('/api/projects/{i}/documents')
def documents(i:str):
 p=load(i);return p.get('documents') or build_documents(p)
...
def main():
    ...
    print('[OK] Project Intelligence module generated and compiled')
    ...
    if MANIFEST.exists():
        body=replace_version(MANIFEST.read_text(encoding='utf-8'))
        body=re.sub(r'EDITION\s*=\s*"[^"]*"','EDITION = "Project Intelligence Platform"',body,count=1)
    ...
    for marker in ('onec_requirement_architect_router','project_intelligence_router','app.include_router(project_intelligence_router)',VERSION):
        if marker not in app:raise RuntimeError(f'6.0.0 missing marker: {marker}')
    print('[OK] API registration validated')
